        ...
        <div class="as-box">
          ...
          <div class="item-as" v-for="(item,index) in asItem" :key="index">
            <div class="item-as-img">
              <img :src="'static/images/'+item.goodsId+'/'+item.goodsImgs" alt="">
            </div>
          ...
          <div class="goods-list-tool">
            <ul>
              <li v-for="(item,index) in goodsTool" :key="index" @click="orderBy(item.en, index)">
                <span :class="{ 'goods-list-tool-active': isAction[index]}">{{item.title}}
  ...
  export default {
    ...
    data() {
      return {
        searchItem: '',
        isAction: [true, false],
        icon: ['arrow-up-a', 'arrow-down-a'],
        goodsTool: [
          {title: '综合', en: 'inventory'},
          {title: '价格', en: 'price'}
        ],
        searchData: {
          'foodstuffName': '',
    ...
    methods: {
      ...
      ...mapMutations(['SET_GOODS_INFO_BY_NAME']),
      orderBy(data, index) {
        console.log(data);
        this.icon = ['arrow-down-a', 'arrow-down-a'];
